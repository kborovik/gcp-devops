...
|T14|x|add `secrets/LOGFIRE_READ_TOKEN.gpg`; patch `claude_code` role — decrypt + render `LOGFIRE_TOKEN` ∈ ubuntu fish init; install Logfire MCP plugin via `claude mcp add` ∨ `~ubuntu/.claude/.mcp.json` template. Claude MAX subscription auth ≡ user-driven post-deploy (`claude` first-run OAuth, ⊥ secrets-managed) — ∴ ⊥ ANTHROPIC_API_KEY threading; `secrets/ANTHROPIC_API_KEY.gpg` remains orphan post-T10, separate cleanup amend|V13,I.cmd

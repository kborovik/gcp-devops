...
- V13: operator-tooling tokens (`LOGFIRE_READ_TOKEN`, `ANTHROPIC_API_KEY` for vim-claude interactive proofread/optimize, ad-hoc query/observability/AI-assist auth on GCE host) ∈ `secrets/` ∧ deploy → ubuntu host filesystem ∨ env allowed; app-runtime tokens (`LOGFIRE_WRITE_TOKEN`, AI-provider keys consumed by mailpilot/leadpilot daemons) ⊥ this repo's responsibility per §V12. Boundary ≡ consumer-shape: operator tokens scoped to interactive ops sessions (SSH + `claude`/`logfire`/vim-claude); app tokens scoped to daemon services — same token name (e.g. `ANTHROPIC_API_KEY`) may legitimately appear on both sides under distinct consumers (operator-side vim-claude allowed; daemon-side ⊥ this repo). Codifies boundary so observability/automation/AI-assist tooling growth ⊥ erodes §V12
- V14: claude_code role applies `ansible/roles/claude_code/files/settings.json` (mirrors developer-laptop config) via mixed dispatch — plugin-lifecycle keys per-key via `claude` CLI, scalar keys via bootstrap copy. Per-key dispatch: `extraKnownMarketplaces.<m>` → `claude plugin marketplace add <source>`; `enabledPlugins.<p>@<m>` → `claude plugin install <p>@<m>`. Scalar keys (`theme`, `editorMode`, `effortLevel`, `autoUpdatesChannel`, `permissions.defaultMode`, `includeCoAuthoredBy`, `skipDangerousModePermissionPrompt`) deployed via `ansible.builtin.copy` w/ `force: false` → bootstraps on first run, ⊥ overwrite ∴ host-side ad-hoc tweaks preserved. Apply order: marketplaces → plugins → settings.json bootstrap. Idempotency: Claude CLI state (plugin lifecycle); `force: false` (scalars). ⊥ local file copy under `files/{commands,skills}/`. Why: per-key CLI delegates plugin lifecycle (install/uninstall/upgrade) to Claude's plugin manager ∧ ⊥ duplicates source-control across two repos (skills) ∨ bypasses MCP-server lifecycle (commands); `claude config set` ⊥ exists ∴ scalars require file copy — `force: false` preserves the no-clobber property that per-key CLI would have given for free

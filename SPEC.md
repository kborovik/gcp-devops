...
- V12: mailpilot config bootstrap @ `~ubuntu/.mailpilot/{config.json,google-credentials.json}` (mode 0600 ubuntu:ubuntu) — `config.json` rendered from `roles/mailpilot/templates/config.json.j2` w/ `{anthropic_api_key, logfire_token}` decrypted from `secrets/{ANTHROPIC_API_KEY,LOGFIRE_TOKEN}.gpg` ∧ threaded as ansible `--extra-vars`; service-account JSON dropped from `secrets/MAILPILOT_GOOGLE_CREDENTIALS.json.gpg`. Non-secret fields (database_url, anthropic_model, pubsub topic/sub, run_interval, logfire_environment) defaulted in template. ⊥ secrets ∈ playbook-vars ∨ role-vars ∨ git-tracked plaintext. Config ∨ keyfile change → notify mailpilot.service restart handler per V11
...
|T7|.|add `ansible/roles/mailpilot/templates/mailpilot.service.j2` ∧ ansible task `template:` → /etc/systemd/system/mailpilot.service (Type=simple, ExecStart={{ mailpilot_home }}/.local/bin/mailpilot run, Restart=on-failure, RestartSec=5, User={{ mailpilot_user }}, After=postgresql.service network-online.target, WantedBy=multi-user.target); handler `systemctl daemon-reload + enable + restart mailpilot`; keep `Initialize database schema` task as pre-start guard|V11
|T8|.|add `secrets/LOGFIRE_TOKEN.gpg` ∧ `secrets/MAILPILOT_GOOGLE_CREDENTIALS.json.gpg`; extend Makefile `mailpilot-deploy` — decrypt 3 secrets ∧ thread as `--extra-vars` (`mailpilot_anthropic_api_key`, `mailpilot_logfire_token`, `mailpilot_google_credentials_json`); add ansible tasks render `config.json.j2` → ~ubuntu/.mailpilot/config.json (0600) ∧ `copy:` google-credentials.json (0600 from --extra-vars); both notify mailpilot.service restart handler|V11,V12
|T9|.|extend Makefile `mailpilot-status` — replace adhoc `mailpilot status` w/ `systemctl is-active mailpilot.service ; mailpilot --version ; journalctl -u mailpilot --no-pager -n 5`; aligns w/ /deploy V6 post-deploy verify|V11,I.cmd
